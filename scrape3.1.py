...
def scanAndScrape():
	...
	if genre is not None:
		...
		loose_price = pc_soup.select('#used_price > span.price')[0].text.strip()
		cib_price = pc_soup.select('#complete_price > span.price')[0].text.strip()
		nib_price = pc_soup.select('#new_price > span.price')[0].text.strip()
...
#			gs_url = "N/A"
		...
		print(title,platform,query,genre,loose_price,cib_price,nib_price)
		print("")
		GameData = ["=\"" + query + "\"",title,platform,genre,loose_price,cib_price,nib_price]
